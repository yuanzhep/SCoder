...
def gradient_based_ho(f0, netD, batch_id):
    ...
    plot_loss(p_list, result_dir + '/curves', 'privacy_{}'.format(batch_id), 'privacy')
...
def hypergradient(tot_loss: torch.Tensor, inv_loss: torch.Tensor, f_adv: torch.Tensor, z: torch.Tensor):
    ...
    v3 = torch.autograd.grad(d_inv_d_z, f_adv, grad_outputs=v2, retain_graph=True) 
...
if __name__ == "__main__":
    ...
    Amor.load_state_dict(torch.load('../params/0125saved.pkl'))
    ...
    for beta in beta_list:
        for i in i_list:
            ...
            for (batch_id, (imgs, label)) in enumerate(dataloader):
                ...
                imgs = imgs.cuda()
                save_tensor_images(imgs.detach(), result_dir+f'/ori/{batch_id}.jpg')
                f0 = Enc(imgs).detach()
