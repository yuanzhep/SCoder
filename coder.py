...
def attack_batch(f0, netG, netD, netEnc, batchID=0):
    ...
    plot_loss(p_list, result_dir + '/curves', 'privacy_{}'.format(batchID), 'loss')
        ...
        for ncritic in Ncritic:
            for beta in Beta:
                for zlr in Zlr:
                    for dlr in Dlr:
                        torch.cuda.empty_cache()
                        result_dir = f'../yz_result/ncritic{ncritic}_beta{beta}_dlr{dlr}_zlr{zlr}/'
